...
void AddFirewallRuleCmdline(LPWSTR exeName, LPWSTR exeFile, LPCWSTR dir)
{
    ...
    if ((intptr_t)hi <= 32) {
        WcaLog(LOGMSG_STANDARD, "Failed to change firewall rule : %d, last error: %d", (intptr_t)hi, GetLastError());
    }
    else {
...
void RemoveFirewallRuleCmdline(LPWSTR exeName)
{
    ...
    // https://learn.microsoft.com/en-us/windows/win32/api/shellapi/nf-shellapi-shellexecutew
    if ((intptr_t)hi <= 32) {
        WcaLog(LOGMSG_STANDARD, "Failed to change firewall rule \"%ls\" : %d, last error: %d", rulename, (intptr_t)hi, GetLastError());
    }
    else {
...
UINT __stdcall AddRegSoftwareSASGeneration(__in MSIHANDLE hInstall)
{
    ...
    // https://learn.microsoft.com/en-us/windows/win32/api/shellapi/nf-shellapi-shellexecutew
    if ((intptr_t)hi <= 32) {
        WcaLog(LOGMSG_STANDARD, "Failed to add registry name \"%ls\", %d, %d", valueName, (intptr_t)hi, GetLastError());
    }
    else {
    ...
    __in MSIHANDLE hInstall)
{
    ...
    // https://learn.microsoft.com/en-us/windows/win32/api/shellapi/nf-shellapi-shellexecutew
    if ((intptr_t)hi <= 32) {
        WcaLog(LOGMSG_STANDARD, "Failed to remove amyuni idd : %d, last error: %d", (intptr_t)hi, GetLastError());
    }
    else {
...
void TryCreateStartServiceByShell(LPWSTR svcName, LPWSTR svcBinary, LPWSTR szSvcDisplayName)
{
    ...
    if ((intptr_t)hi <= 32) {
        WcaLog(LOGMSG_STANDARD, "Failed to create service with shell : %d, last error: 0x%02X.", (intptr_t)hi, GetLastError());
    }
    else {
    ...
    if ((intptr_t)hi <= 32) {
        WcaLog(LOGMSG_STANDARD, "Failed to start service with shell : %d, last error: 0x%02X.", (intptr_t)hi, GetLastError());
    }
    else {
...
void TryStopDeleteServiceByShell(LPWSTR svcName)
{
    ...
    hi = ShellExecuteW(NULL, L"open", L"cmd.exe", szCmd, NULL, SW_HIDE);
    if ((intptr_t)hi <= 32) {
        WcaLog(LOGMSG_STANDARD, "Failed to delete service with shell : %d, last error: 0x%02X.", (intptr_t)hi, GetLastError());
    }
    else {
